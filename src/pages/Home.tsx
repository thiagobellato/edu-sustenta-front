...
interface DashboardStats {
  ...
  trilhas_publicadas?: number;
  trilhas_rascunho?: number;
...
export default function Home() {
  ...
  // Redireciona usuários USER para escolher papel
  useEffect(() => {
  ...
  const { data: stats, isLoading } = useQuery<DashboardStats>({
    ...
    enabled: user?.role !== 'user', // Não busca stats se for USER
  });
  ...
  // Se for USER, não renderiza nada (será redirecionado)
  if (user?.role === 'user') {
    return null;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard title="Total de Trilhas" value={stats?.minhas_trilhas ?? 0} icon={BookOpen} />
        <StatsCard title="Trilhas Publicadas" value={stats?.trilhas_publicadas ?? 0} icon={CheckCircle} />
      </div>
      
      {stats?.trilhas_rascunho !== undefined && stats.trilhas_rascunho > 0 && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="font-semibold text-amber-900">
                  Você tem {stats.trilhas_rascunho} trilha{stats.trilhas_rascunho > 1 ? 's' : ''} em rascunho
                </p>
                <p className="text-sm text-amber-700">
                  Finalize e publique suas trilhas para que os alunos possam acessá-las
                </p>
              </div>
              <Button asChild variant="outline" size="sm" className="border-amber-300 text-amber-900 hover:bg-amber-100">
                <Link to="/teacher-trails">
                  Ver Trilhas
            ...
            <div className="space-y-3">
              {stats.recentActivity.map((activity) => (
                <div key={activity.id} className="flex items-start gap-3 pb-3 border-b last:border-0">
                  <div className="mt-1 flex h-2 w-2 rounded-full bg-[#012030]" />
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#012030]">{activity.text}</p>
                    <p className="text-xs text-[#012030]/60">{activity.time}</p>
                  </div>
                </div>
              ))}
            </div>
